  ...
  `;
	...
	while (true) {
		...
		while (actionQueue.length > 0) {
			...
			case ("Init"):
				ns.exec("initBN.js", "home", 1);
				break;
			case ("DWAll"):
				ns.exec("utils/darkweb.auto.js", "home", 1);
				break;
			case ("batch1"):
				toggleSimple(ns, action, 10);
				break;
			case ("Dashboard"):
				toggleSimple(ns, action, 10);
				break;
...
function toggleSimple(ns, action, argument) {
	...
	if (action.enable) {
		if (argument)
			ns.exec(script, "home", 1, argument);
		else
